...
# With for_each we can create multiple resources with different configurations
...
resource "aws_iam_user" "iam_users" {
  for_each = var.iam_users # We can use a set or a map
  name     = each.value    # We can access the value with each.value
  path     = "/users/"
}
...
resource "aws_iam_user" "iam_users_from_map" {
  for_each = var.iam_users # We can use a set or a map
  name     = each.key      # We can access the key with each.key
  path     = "/users/"
}
